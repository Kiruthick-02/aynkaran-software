...
function StageField({ label, field, type = 'text', options, formData, onChange }) {
	...
	return (
		<label className="block">
			...
			{options ? (
				...
				<input type={type} value={formData[field] || ''} onChange={(e) => onChange?.(field, e.target.value)} className="mt-1 w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-white outline-none focus:border-blue-500" />
			)}
		</label>
	);
}
...
export default function StageActivityPanel({
	...
	return (
		<section className="p-5 bg-slate-900/70 border border-slate-800 rounded-2xl space-y-4">
			...
			{currentStageNumber === 2 && <div className="grid grid-cols-1 md:grid-cols-3 gap-3"><StageField formData={formData} onChange={onChange} label="Commitment status" field="commitmentStatus" options={['Pending', 'Signed', 'Declined']} /><StageField formData={formData} onChange={onChange} label="Declaration document reference" field="commitmentDocument" /></div>}
			{currentStageNumber === 4 && <div className="grid grid-cols-1 md:grid-cols-3 gap-3"><StageField formData={formData} onChange={onChange} label="PRL application status" field="prlApplicationStatus" options={['Pending', 'Applied', 'Completed']} /><StageField formData={formData} onChange={onChange} label="Application date" field="prlApplicationDate" type="date" /><StageField formData={formData} onChange={onChange} label="Application/reference number" field="prlReference" /><StageField formData={formData} onChange={onChange} label="Application document reference" field="prlDocument" /></div>}
			{[5, 7].includes(currentStageNumber) && <div className="grid grid-cols-1 md:grid-cols-3 gap-3"><StageField formData={formData} onChange={onChange} label="Fee type" field="feeType" /><StageField formData={formData} onChange={onChange} label="Amount" field="amount" type="number" /><StageField formData={formData} onChange={onChange} label="Payment status" field="paymentStatus" options={['Pending', 'Paid']} /><StageField formData={formData} onChange={onChange} label="Payment date" field="paymentDate" type="date" /><StageField formData={formData} onChange={onChange} label="Payment method" field="paymentMethod" options={['Cash', 'Bank', 'UPI', 'Other']} /><StageField formData={formData} onChange={onChange} label="Reference number" field="paymentReference" /></div>}
			{currentStageNumber === 6 && <div className="grid grid-cols-1 md:grid-cols-3 gap-3"><StageField formData={formData} onChange={onChange} label="Enrollment status" field="trainingEnrollmentStatus" options={['Pending', 'Enrolled', 'Active']} /><StageField formData={formData} onChange={onChange} label="Enrollment date" field="trainingEnrollmentDate" type="date" /><StageField formData={formData} onChange={onChange} label="Training reference number" field="trainingReference" /><StageField formData={formData} onChange={onChange} label="Training batch" field="trainingBatch" /><StageField formData={formData} onChange={onChange} label="Training start date" field="trainingStartDate" type="date" /><StageField formData={formData} onChange={onChange} label="Training end date" field="trainingEndDate" type="date" /><StageField formData={formData} onChange={onChange} label="Training provider / portal" field="trainingProvider" /></div>}
			{currentStageNumber === 8 && <div className="grid grid-cols-1 md:grid-cols-3 gap-3"><StageField formData={formData} onChange={onChange} label="Exam registration number" field="examRegistrationNumber" /><StageField formData={formData} onChange={onChange} label="Exam registration date" field="examRegistrationDate" type="date" /><StageField formData={formData} onChange={onChange} label="Exam date" field="examDate" type="date" /><StageField formData={formData} onChange={onChange} label="Exam center" field="examCenter" /><StageField formData={formData} onChange={onChange} label="Hall ticket/document reference" field="hallTicketReference" /><StageField formData={formData} onChange={onChange} label="Exam fee" field="amount" type="number" /><StageField formData={formData} onChange={onChange} label="Payment status" field="paymentStatus" options={['Pending', 'Paid']} /><StageField formData={formData} onChange={onChange} label="Payment date" field="paymentDate" type="date" /><StageField formData={formData} onChange={onChange} label="Payment method" field="paymentMethod" options={['Cash', 'Bank', 'UPI', 'Other']} /><StageField formData={formData} onChange={onChange} label="Payment reference" field="paymentReference" /></div>}
			{currentStageNumber === 9 && <div className="grid grid-cols-1 md:grid-cols-3 gap-3"><StageField formData={formData} onChange={onChange} label="Certification result" field="result" options={['Pending', 'Passed', 'Failed']} /><StageField formData={formData} onChange={onChange} label="Result date" field="resultDate" type="date" /><StageField formData={formData} onChange={onChange} label="Certification number" field="certificationNumber" /><StageField formData={formData} onChange={onChange} label="Score" field="score" /></div>}
